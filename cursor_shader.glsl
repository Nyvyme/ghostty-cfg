...
vec4 hex_to_vec4(uint col) {
  float r = float((col >> 16) & 0xFF) / float(0xFF);
  float g = float((col >> 8) & 0xFF) / float(0xFF);
  float b = float(col & 0xFF) / float(0xFF);
  float a = 1.0;

  vec4 vCol = vec4(r, g, b, a);
  return vCol;
}
